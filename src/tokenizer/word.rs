...
impl<'a> Words<'a> {
    ...
    /// Create new iterator for *words* and treat the symbols in the alphabet as letters.
    #[inline]
    pub fn new_with_alphabet(s: &str, filter: fn(&&str) -> bool, alphabet: Vec<char>) -> Words<'_> {
        Words {
            inner: WordBounds::new_with_alphabet(s, alphabet).filter(filter),
        }
    }
}
...
impl<'a> WordBoundIndices<'a> {
    /// Create new iterator for *word boundaries and their indices*.
    #[inline]
    ...
    /// Create new iterator for *word boundaries and their indices*.
    #[inline]
    pub fn new_with_alphabet(s: &'a str, alphabet: Vec<char>) -> WordBoundIndices<'a> {
        WordBoundIndices {
            start_offset: s.as_ptr() as usize,
            iter: WordBounds::new_with_alphabet(s, alphabet),
        }
    }
...
impl<'a> WordBounds<'a> {
    ...
    #[inline]
    pub fn new(s: &'a str) -> WordBounds<'a> {
        WordBounds {
            string: s,
            cat: None,
            catb: None,
    ...
    /// Create new iterator for *word boundries* with an alphabet.
    #[inline]
    pub fn new_with_alphabet(s: &'a str, alphabet: Vec<char>) -> WordBounds<'a> {
        WordBounds {
            string: s,
            cat: None,
            catb: None,
