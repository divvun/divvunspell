...
#[no_mangle]
pub extern fn word_bound_indices(utf8_string: *const c_char) -> *mut CWordBoundIndices {
    let c_str = unsafe { CStr::from_ptr(utf8_string) };
    let string = c_str.to_str().unwrap().to_string();

    let mut thing = CWordBoundIndices { string, iterator: unsafe { std::mem::uninitialized() } };
    thing.iterator = unsafe { std::mem::transmute(thing.string.word_bound_indices()) };
    Box::into_raw(Box::new(thing))
}

#[no_mangle]
pub extern fn word_bound_indices_next(handle: *mut CWordBoundIndices, out_index: *mut u64, out_string: *mut *mut c_char) -> bool {
...
#[no_mangle]
pub extern fn word_bound_indices_free(handle: *mut CWordBoundIndices) {
    unsafe { Box::from_raw(handle) };
}
